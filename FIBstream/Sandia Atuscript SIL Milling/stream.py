...
class Path():
    ...
    def render(self,pos,scale,rep=1,trunc=1):
        """
        ...
        out_of_bounds = 0
        for point in c:
            if not (0<=point[1]<65536 and 0<=point[2]<56574): # Asher:  it seems like only their x bound is 0 - 65536, but our bound is 65535 for both 
               out_of_bounds += 1
            s += '%i %i %i\n'%tuple(point)
        if out_of_bounds:
...
class Scene():
    ...
    *args:
    ...
    def rep_and_trunc(self, rep=1):
        ...
        rep_min = int(longest/(2**18))+1 # (longest+1)/(4096*64)+1 
        if rep_min > rep:
